...
export default function MetadataSection({ drug }: MetadataSectionProps) {
  ...
  return (
    <div id="supplementary-metadata" className="space-y-4 pt-6 border-t border-gray-200 dark:border-gray-800">
      {/* 1. Other Ingredients (Collapsed by default) */}
      <details className="group rounded-xl border border-gray-200 bg-gray-50/60 transition-colors open:bg-white dark:border-gray-800 dark:bg-gray-900/40 dark:open:bg-gray-900">
        <summary className="flex cursor-pointer list-none items-center justify-between p-4 text-sm font-semibold text-gray-800 select-none dark:text-gray-200">
          <span>Other ingredients (Inactive)</span>
          <svg
            className="h-4 w-4 text-gray-500 transition-transform duration-200 group-open:rotate-180 dark:text-gray-400"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth={2}
        ...
        <div className="border-t border-gray-200/60 p-4 text-xs leading-relaxed text-gray-600 dark:border-gray-800 dark:text-gray-400">
      ...
      <details className="group rounded-xl border border-gray-200 bg-gray-50/60 transition-colors open:bg-white dark:border-gray-800 dark:bg-gray-900/40 dark:open:bg-gray-900">
        <summary className="flex cursor-pointer list-none items-center justify-between p-4 text-sm font-semibold text-gray-800 select-none dark:text-gray-200">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded bg-gray-200 px-1.5 py-0.5 text-[10px] font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">
              Technical
            </span>
          </div>
          <svg
        ...
        <div className="border-t border-gray-200/60 p-4 text-xs leading-relaxed dark:border-gray-800">
          ...
          <dl className="grid grid-cols-1 gap-x-4 gap-y-2 sm:grid-cols-2">
            {metadataItems.map((item) => (
              <div key={item.label} className="border-b border-gray-100 pb-1.5 dark:border-gray-800/60">
                <dt className="text-gray-500 dark:text-gray-400">{item.label}</dt>
                <dd className="font-mono text-gray-800 dark:text-gray-200">{item.value}</dd>
              </div>
            ))}
          </dl>
        </div>
